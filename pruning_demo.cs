...
Console.WriteLine($"\nBegin unstructured neural network pruning demo\n");
    ...
    int len, float lr, float mom, float threshold, float FACTOR, int EPOCHS, int BATCHSIZE)
{
    ...
    for (int epoch = 0, B = len / BATCHSIZE; epoch < EPOCHS; epoch++, lr *= FACTOR, mom *= FACTOR)
    {
        ...
        if (epoch == EPOCHS / 2)
        {
            Console.WriteLine($"\nMagnitude pruning with {threshold:F2} threshold after {epoch} epochs");
            PruningUnstructured(positions, weights, deltas, threshold);
            Console.WriteLine($"Final weights size = {GetWeightsSize(weights)}");
            Console.WriteLine($"Final positions size = {GetWeightsSize(positions)}\n");
        }
...
static (int[] net, int[][] positions, float[][] weights) LoadPruningNetFromFile(string fileName)
{
    ...
    {
        string line = lines[i];
        positions[i - 1] = line == "" ? Array.Empty<int>() : Array.ConvertAll(line.Split(','), int.Parse);
    }

    // Step 3: Read the weights array lines
    for (int i = 1 + numPositionsLines; i < lines.Length; i++)
    {
        string line = lines[i];
        weights[i - 1 - numPositionsLines] = line == "" ? Array.Empty<float>() : Array.ConvertAll(line.Split(','), float.Parse);
    }

    return (net, positions, weights);
}
// todo: 9. fit the pruned architecture to used values only (structured pruning)
